...
class CatBoostWRUL(SupervisedMethodInterface):
    ...
    def fit(self, historic_data: list[pd.DataFrame], historic_sources: list[str], event_data: pd.DataFrame,
        ...
        for current_historic_data, current_historic_source, labels in zip(historic_data, historic_sources,
                                                                          anomaly_ranges):
            self.model_per_source[current_historic_source] = CatBoostRegressor(*self.initial_args,
                                                                               **self.initial_kwargs)
            if self.seq_len>1:
            ...
            if self.save_model:
                import pickle
                with open(f"catboost_w_model_{current_historic_source}.pkl", "wb") as f:
    ...
    def get_params(self) -> dict:
        return {
            **(CatBoostRegressor(*(self.initial_args), **(self.initial_kwargs)).get_params()),
